...
def _get_semaphore():
    ...
    if _GLOBAL_TEST_SEMAPHORE is None:
        _GLOBAL_TEST_SEMAPHORE = asyncio.Semaphore(2)
...
def _execute_function_test_in_subprocess(wrapper_code: str, expected_output_str: str,
                                         memory_limit_mb: int, conn) -> None:
    """
    Worker function to execute function test in subprocess with resource limits
    Uses wrapper code generated by generate_function_wrapper
    This needs to be at module level for multiprocessing to pickle it
    """
    result = None
    try:
        ...
        # Execute wrapper code and capture output
        import sys
        import io
        ...
        # Parse wrapper output (JSON format)
        result_data = json.loads(output.strip())

        if not result_data.get("success", False):
            result = (False, result_data.get("error", "Execution failed"))
        else:
            # Compare results using main branch logic
            exec_outputs = result_data["result"]
            test_case_outputs = json.loads(expected_output_str)
                ...
                try:
                    if isinstance(exec_outputs[0], tuple):
                        ...
                        if exec_outputs == test_case_outputs[0]:
                            ...
                            result = (False, f"Expected {test_case_outputs}, got {exec_outputs}")
                    else:
                        result = (False, f"Expected {test_case_outputs}, got {exec_outputs}")
                except:
                    result = (False, f"Expected {test_case_outputs}, got {exec_outputs}")
        ...
        result = (False, f"JSON parse error: {str(e)}")
    except Exception as e:
        result = (False, f"{type(e).__name__}: {str(e)}")
    finally:
        # Always try to send result
...
def _run_function_test_subprocess(code: str, fn_name: str, test_input_str: str,
    ...
    """
    # Generate wrapper code using main branch's generate_function_wrapper
    ...
    process = multiprocessing.Process(
        ...
        args=(wrapper_code, expected_output_str, memory_limit_mb, child_conn)
    )
    process.start()
...
class CodeTask:
    ...
    async def _run_function_test(
        ...
        try:
            # Prepare test_input for generate_function_wrapper
            # If test_input is a list, convert to newline-separated string format
            ...
            test_passed, error_msg = await loop.run_in_executor(
                ...
                code,  # Don't add BASE_IMPORTS here, wrapper does it
                fn_name,
                test_input_str,
                expected_output,  # This is the JSON string
                memory_limit_mb,
                timeout
            )
